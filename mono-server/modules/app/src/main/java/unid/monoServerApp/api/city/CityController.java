package unid.monoServerApp.api.city;//package unid.monoServerApp.api.country;
...
public class CityController {
    ...
    public @Valid UnifiedResponse<PaginationResponse<CityResponse>> list(
            @Valid
            @ParameterObject
            PaginationRequest payload
    ) {
        var table = CityPagination.TABLE;
        var dslContext = dbCity.getDsl();
        var extraConditionOnFirstPage = DSL.noCondition();
        if (StringUtils.isBlank(payload.getPage())) {
            extraConditionOnFirstPage = dbCity.validateCondition(table);
        }
        var result = PaginatedQuery.init(
                ...
                .select(dsl -> dbCity.getQuery(table))
                .conditions(CityPagination.conditionVisitor)
                .extraConditions(extraConditionOnFirstPage)
                .sortBy(
                        CityPagination.orderingVisitor,
                        null,
                        null,
                        null,
                        uniqueSort -> {
        ...
        return UnifiedResponse.of(
                PaginationResponse.asResult(result, cityMapper.toResponse(result.getResult().into(DbCity.Result.class)))
        );
    }
    ...
    public @Valid UnifiedResponse<CityResponse> get(
            @PathVariable("id") UUID id
    ) {
        var result = cityService.get(id);
        return UnifiedResponse.of(
                cityMapper.toResponse(result)
        );
    }
    ...
    public @Valid UnifiedResponse<CityResponse> create(
            @RequestBody @Valid
            CityRequest payload
    ) {
        var result = cityService.get(
                cityService.create(payload)
                        .getId()
        );
        var resp = cityMapper.toResponse(result);
        return UnifiedResponse.of(
                resp
        );
    }
    ...
    public @Valid UnifiedResponse<CityResponse> update(
            @PathVariable("id") UUID id,
            @RequestBody @Valid
            CityRequest payload
    ) {
